...
// MasterChefV2 is the master of Swpy. He can make Swpy and he is a fair guy.
//
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    function pendingSwpy(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accSwpyPerShare = accSwpyPerShare.add(swpyReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 swpyReward = multiplier.mul(swpyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        swpy.mint(devaddr, swpyReward.div(10));
        swpy.mint(address(this), swpyReward);
        pool.accSwpyPerShare = pool.accSwpyPerShare.add(swpyReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeSwpyTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > swpyBal) {
            transferSuccess = swpy.transfer(_to, swpyBal);
        } else {
            transferSuccess = swpy.transfer(_to, _amount);
        }
		
        require(transferSuccess, "safeSwpyTransfer: transfer failed");
    }
    ...
    //HappySwap has to add hidden dummy pools inorder to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _swpyPerBlock) public onlyOwner {
        massUpdatePools();
        swpyPerBlock = _swpyPerBlock;
		
		emit UpdateEmissionRate(msg.sender, _swpyPerBlock);
    }
}
