...
// File: @happyswapfinance/happyswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/HappySwapProfile.sol

pragma solidity ^0.6.0;

/** @title HappySwapProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract HappySwapProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public swpyToken;
    ...
    uint256 public numberSwpyToReactivate;
    uint256 public numberSwpyToRegister;
    uint256 public numberSwpyToUpdate;
    ...
    constructor(
        IBEP20 _swpyToken,
        uint256 _numberSwpyToReactivate,
        uint256 _numberSwpyToRegister,
        uint256 _numberSwpyToUpdate
    ) public {
        swpyToken = _swpyToken;
        numberSwpyToReactivate = _numberSwpyToReactivate;
        numberSwpyToRegister = _numberSwpyToRegister;
        numberSwpyToUpdate = _numberSwpyToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends SWPY to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer SWPY tokens to this contract
        swpyToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberSwpyToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer SWPY token to this address
        swpyToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberSwpyToUpdate
    ...
    /**
     * @dev Claim SWPY to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        swpyToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of SWPY to register
     * Callable only by owner admins.
     */
    function updateNumberSwpy(
        uint256 newNumberSwpyToReactivate,
        uint256 _newNumberSwpyToRegister,
        uint256 _newNumberSwpyToUpdate
    ) external onlyOwner {
        numberSwpyToReactivate = newNumberSwpyToReactivate;
        numberSwpyToRegister = _newNumberSwpyToRegister;
        numberSwpyToUpdate = _newNumberSwpyToUpdate;
    }
